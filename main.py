...
# Module-level selected MA summary (set by main run)
SELECTED_MA_TYPE = None
SELECTED_MA_LENGTH = None
SELECTED_MA_FINAL_CASH = None
SELECTED_MA_DETAILS = None
...
def get_settings():
    ...
    min_len = ask("Min MA length to scan (int)", int, 1)
    ...
    ranking = input("Ranking method ('points' or 'final_cash') [final_cash]: ") or 'final_cash'
    return {
        'ticker': ticker,
        'period': period,
        'interval': interval,
...
def clamp_settings(settings):
    # enforce sane limits
    settings['min_len'] = max(1, min(200, int(settings.get('min_len', 1))))
    settings['max_len'] = max(settings['min_len'], min(200, int(settings.get('max_len', 30))))
...
def scan_all_mas_single(df, min_len=1, max_len=30, tp_sl_ratio=1.5, max_holding=60, sl_offset_dollars=1.0, tp_dollars=1.5, initial_cash=10000.0):
    ...
    # we'll use final_cash (index 3) as the ranking metric
    overall_best = (None, None, 0, float(initial_cash), -np.inf, 0.0, 0.0, 0)
    for t in types:
        for length in range(min_len, max_len + 1):
            ...
            # rank by final_cash primarily, then total_pct
            if num_trades_s > 0:
                if (final_cash_s > overall_best[3]) or (final_cash_s == overall_best[3] and total_pct_s > overall_best[4]):
                    overall_best = (t, length, 0, final_cash_s, total_pct_s, dollars_gained_s, dollars_lost_s, num_trades_s)
...
if __name__ == '__main__':
    ...
    print(f"\nSingle-MA TP/SL scan results (length {settings.get('min_len',1)}..{settings['max_len']}):")
    for t, rows in results.items():
        # prefer highest final_cash then number of trades
        def keyfn(r):
            return (float(scalarize(r[6], 0.0)), float(scalarize(r[2], 0.0)))
    ...
    print(f"{t}: best length={length}, total_pct={total_pct:.4f}, trades={num_trades}, final_cash=${final_cash_b:.2f}, gained=${dollars_gained_b:.2f}, lost=${dollars_lost_b:.2f}")

    # recompute overall_best by final_cash if requested
    if settings.get('ranking', 'final_cash') == 'final_cash':
        best_fc = None
        for t, rows in results.items():
            for r in rows:
                # r == (length, total_pct_s, num_trades_s, dollars_gained_s, dollars_lost_s, points_s, final_cash_s)
    ...
    # set module-level selection variables for user convenience
    if overall_best[0] is not None:
        # assign module-level selection variables via globals() to avoid scope issues
        globals()['SELECTED_MA_TYPE'] = overall_best[0]
        globals()['SELECTED_MA_LENGTH'] = int(overall_best[1])
        globals()['SELECTED_MA_FINAL_CASH'] = float(overall_best[3])
        globals()['SELECTED_MA_DETAILS'] = {
            'points': int(overall_best[2]),
